...
public class Running extends Layer{
	...
	protected Running(int x, int y, int width, int height, GameRunning running, JPanelGame panel) {
	...
	public void paint(Graphics g) {
		...
		while((int)day>0) {
			int num = day%10;
			g.drawImage(numberIMG[num], posX, posY, posX+numberIMG[num].getWidth(null),
					posY+numberIMG[num].getHeight(null), 0, 0, numberIMG[num].getWidth(null), numberIMG[num].getHeight(null), null);
			day /= 10;
			posX -= 26;
		}
		...
		if(GameRunning.GAME_DAY>0) {
			posY += 14;
			posX = 100;
			g.drawImage(bgDay, 0, posY, bgDay.getWidth(null), posY+bgDay.getHeight(null),
					0, 0, bgDay.getWidth(null), bgDay.getHeight(null), null);
			int day_m = GameRunning.GAME_DAY;
			posY += 16;
			while ((int)day_m>0) {
				int num = day_m%10;
				g.drawImage(numberR[num], posX, posY, posX+numberR[num].getWidth(null), posY+numberR[num].getHeight(null), 0, 0,numberR[num].getWidth(null), numberR[num].getHeight(null), null);
				day_m /= 10;
				posX -= 26;
			}
		}
